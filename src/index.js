...
function findAllPSiblings(where) {
  ...
  for (var i = 0; i < where.children.length; i++) {
    if (where.children[i].matches('p')) {
      NewArr.push(where.children[i - 1])
    }
  }
...
function deleteTextNodesRecursive(where) {
  for (let childNode of [...where.childNodes]) {
  ...
  if (where.nodeType === document.ELEMENT_NODE) {
    for (let i = 0; i < where.childNodes.length; i++) {
      if (deleteTextNodesRecursive(where.childNodes[i])) {
        TextNodes.push(where.childNodes[i]);
      }
    }
...
function collectDOMStat(root) {
  ...
  function collectStatistics(root) {
    for (let childNode of [...root.childNodes]) {
      if (childNode.nodeType === 1) {
        statistics.tags[childNode.tagName] = (statistics.tags[childNode.tagName] || 0) + 1;
        /* if (childNode.tagName in statistics.tags) {
          statistics.tags[childNode.tagName] += 1;
        } else {
          statistics.tags[childNode.tagName] = 1;
        } */
        for (let Class of childNode.classList) {
          //statistics.classes[Class] === undefined ? statistics.classes[Class] = 1 : statistics.classes[Class]++;
          statistics.classes[Class] = (statistics.classes[Class] || 0) + 1;
        }
        collectStatistics(childNode);
      } else if (childNode.nodeType === 3) {
        statistics.texts++;
      }
    }
